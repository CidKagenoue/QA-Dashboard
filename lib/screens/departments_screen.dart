  ...
  Future<void> _deleteDepartment(Department dept) async {
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            child:
                const Text('Verwijderen', style: TextStyle(color: Colors.white)),
          ),
        ],
      ),
    );
    ...
    if (confirm == true) {
      ...
      try {
        ...
        if (_selected?.id == dept.id) _selected = null;
        await _loadData();
      } finally {
        if (mounted) setState(() => _isLoading = false);
      }
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDialogState) {
          ...
          final filtered = allUsers
              .where((u) =>
                  (u.name ?? u.email).toLowerCase().contains(query))
              .toList();
              ...
              itemBuilder: (context, index) {
                ...
                return Container(
                  ...
                  child: ListTile(
                    ...
                    // ✅ FIX: geen navigatie meer, alleen selectie updaten
